...
- ОБЯЗАТЕЛЬНО заполняй массив "relations": связи между сущностями кладутся ТОЛЬКО туда,
  каждая как {{"type": <тип связи>, "from": <name сущности>, "to": <name сущности>}};
  НЕЛЬЗЯ писать связи как ключи внутри entity (например "HAS_GEO": "...") — это будет отброшено;
- поля "from" и "to" в relations должны ТОЧНО совпадать с "name" из массива entities;
- связывай процесс с его материалами (USES_MATERIAL), условиями (OPERATES_AT_CONDITION),
  результатами/выводами (PRODUCES_OUTPUT), оборудованием (USES_EQUIPMENT), публикацией
  (DESCRIBED_IN) и географией (HAS_GEO).
Формат ответа — ТОЛЬКО JSON, пример структуры (значения замени фактами из текста):
{{"entities": [
   {{"type": "Process", "name": "флотация шлака"}},
   {{"type": "Material", "name": "медь"}},
   {{"type": "Publication", "name": "Иванов И.И. Флотация шлаков, 2021", "props": {{"year": 2021}}}},
   {{"type": "GeoContext", "name": "Россия"}}],
 "relations": [
   {{"type": "USES_MATERIAL", "from": "флотация шлака", "to": "медь"}},
   {{"type": "DESCRIBED_IN", "from": "флотация шлака", "to": "Иванов И.И. Флотация шлаков, 2021"}},
   {{"type": "HAS_GEO", "from": "Иванов И.И. Флотация шлаков, 2021", "to": "Россия"}}],
 "conditions": [
   {{"param": "recovery", "op": ">=", "value": 90, "unit": "%"}}]}}"""
